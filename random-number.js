...
/* Coin toss v2 — smooth physical-feeling flick and continuous 3D rotation */
.coin-stage-v2{min-height:455px!important;position:relative;overflow:hidden;border-radius:24px;background:radial-gradient(circle at 52% 38%,#ffffff 0,#eef5ff 46%,#dfe9f6 100%);display:grid;place-items:center;isolation:isolate}
.coin-stage-v2::before{content:"";position:absolute;inset:auto 8% 24px;height:70px;border-radius:50%;background:radial-gradient(ellipse at center,rgba(65,105,160,.22),rgba(65,105,160,0) 68%);filter:blur(10px)}
.coin-air-glow{position:absolute;top:42px;left:50%;width:170px;height:170px;transform:translateX(-5%);border-radius:50%;background:radial-gradient(circle,rgba(255,255,255,.95),rgba(154,198,255,.16) 45%,rgba(255,255,255,0) 72%);pointer-events:none;z-index:0}
.coin-launch-scene-v2{position:relative;width:470px;height:370px;z-index:2}
.hand-flick-v2{position:absolute;left:52px;bottom:12px;width:230px;height:238px;transform-origin:72% 86%;filter:drop-shadow(0 18px 14px rgba(25,45,70,.13));z-index:3}
.hand-flick-v2 svg{display:block;width:100%;height:100%}
.hand-flicking{animation:handFlickSmooth .26s cubic-bezier(.22,.82,.18,1) forwards}
.coin-flight-v2{position:absolute;left:154px;top:42px;width:112px;height:112px;z-index:4;transform:translate3d(0,0,0)}
.coin-visual-v2{position:relative;width:112px;height:112px;transform-style:preserve-3d;filter:drop-shadow(0 20px 10px rgba(52,68,84,.22));will-change:transform}
.coin-face-v2{position:absolute;inset:0;border-radius:50%;display:grid;place-items:center;backface-visibility:hidden;-webkit-backface-visibility:hidden;border:5px solid #9c651e;background:radial-gradient(circle at 34% 28%,#fff3be 0,#eab64b 38%,#b66f17 74%,#8d5614 100%);box-shadow:inset 0 0 0 3px rgba(255,248,216,.72),inset -10px -12px 18px rgba(88,45,0,.2),0 2px 0 rgba(255,255,255,.35)}
.coin-face-v2::after{content:"";position:absolute;inset:11px;border-radius:50%;border:2px solid rgba(115,69,8,.34);box-shadow:inset 0 0 0 2px rgba(255,240,180,.24)}
.coin-front-v2{transform:rotateY(0deg)}
.coin-back-v2{transform:rotateY(180deg)}
.coin-letter{position:relative;z-index:2;font-family:Georgia,'Times New Roman',serif;font-size:4.5rem;font-weight:900;line-height:1;color:#6c3f0d;text-shadow:0 2px 0 rgba(255,244,199,.65)}
.coin-spark{animation:coinSpark .42s ease-out}
.coin-settle{animation:coinSettle .28s cubic-bezier(.2,.7,.2,1)}
.coin-instruction-v2{position:absolute;left:50%;bottom:7px;transform:translateX(-50%);padding:10px 15px;border-radius:999px;background:rgba(255,255,255,.86);backdrop-filter:blur(8px);font-size:.86rem;font-weight:800;white-space:nowrap;z-index:6;box-shadow:0 8px 22px rgba(40,70,110,.08)}
.coin-status-text{text-align:center;margin:.45rem 0 1rem;color:#5f6b7b;font-size:.92rem;min-height:1.4em}
.coin-result-show{animation:resultPop .45s ease-out}
@keyframes handFlickSmooth{0%{transform:rotate(0) translateY(0)}45%{transform:rotate(-5deg) translateY(-3px)}100%{transform:rotate(5deg) translateY(-1px)}}
@keyframes coinSettle{0%{transform:scale(1.08)}70%{transform:scale(.95)}100%{transform:scale(1)}}
@keyframes coinSpark{0%{filter:drop-shadow(0 20px 10px rgba(52,68,84,.22)) brightness(1)}50%{filter:drop-shadow(0 18px 14px rgba(52,68,84,.18)) brightness(1.16)}100%{filter:drop-shadow(0 20px 10px rgba(52,68,84,.22)) brightness(1)}}
@media(max-width:700px){.coin-stage-v2{min-height:390px!important}.coin-launch-scene-v2{transform:scale(.82);transform-origin:50% 92%}.coin-air-glow{transform:translateX(-10%) scale(.85)}.coin-instruction-v2{font-size:.76rem}.coin-status-text{font-size:.84rem}}
@media(prefers-reduced-motion:reduce){.hand-flicking,.coin-settle,.coin-spark{animation:none!important}.coin-visual-v2{transition:none!important}}
